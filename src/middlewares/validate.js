...
exports.validateTour = async function (req, res, next) {
    ...
    try {
        ...
        req.tour = tour;
        next()
    } catch (error) { return res.status(500).json({ status: "failed to get TOUR ID", error: err.message }); }
...
exports.validateCatParams = async function (req, res, next) {
    ...
    console.log("CATEGORY ID from validate.js :", catId)
    try {
...
exports.validateReview = async function (req, res, next) {
    ...
    console.log("REVIEW ID from validate.js :", reviewId)
    try {
        const review = await Review.findById(reviewId);
        if (!review) return res.status(404).json({ status: "failed to get REVIEW ID", error: "ERROR!" });
        req.review = review;
        next()
    } catch (error) { return res.status(500).json({ status: "failed to get REVIEW ID", error: "too bad" }); }
}
