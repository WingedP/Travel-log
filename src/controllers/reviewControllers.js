...
try{   
    const reviewNumPerTour = await Review.find({tour:tourId}).countDocuments();
    const reviewFromOneTour=await Review.find({tour:tourId})
    .populate("reviews") .populate("user","_id name email");
    res.status(200).json({ status: "success", numberOfReview:reviewNumPerTour, data: reviewFromOneTour})
...
//       const {rId} = req.params;
//       try {
//       const updateReview = await Review.findByIdAndUpdate(rId,{title:req.body.title,content:req.body.content,rating:req.body.rating},{new:true})
...
// //(WRONG CODES: )READ ALL REVIEWS FROM 1 TOUR: 
// exports.readReviews = async function (req, res){
//   try{   
